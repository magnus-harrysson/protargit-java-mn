...
public class AutoFlip implements Runnable, PropertyChangeListener {
    ...
    public AutoFlip(MechanicalService motorAndMovementHandler, McuService mcuService, OsService osService, BroadcasterService broadcasterService){
    ...
    public void run() {
        try {
            ...
            mcuService.writeSerial(new McuEvent(McuCommand.FLIP_ANGLE, motorAndMovementHandler.getFlipAutoMovement().getShowAngle()));
            mcuService.writeSerial(new McuEvent(McuCommand.HIT_ZONES, McuDataLimit.HIT_ZONES_ENABLE.getValue()));
            osService.playAudioMp3("flip.mp3");
        ...
        while(!exit) {
            ...
            if(hitDetected) {
                ...
                threadSleep(motorAndMovementHandler.getFlipAutoMovement().getHideTime());
